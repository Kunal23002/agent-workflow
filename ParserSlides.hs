...
--   written in the low-level Parsec style from the lecture slides
--   (CS252 Day 12).  Combinators are built up from 'many1', 'digit',
--   'char', 'string', '<|>', 'try', and '<?>' rather than relying on
--   the high-level @Text.Parsec.Token@ token builder used in
--   'Parser'.
--
--   Both parsers produce the same 'Stmt' AST and either can drive the
--   interpreter; the original 'Parser' module is kept untouched as a
--   backup.
module ParserSlides
  ( parseProgram
  ) where
...
-- | A double-quoted string.  No escape sequences for now (matches the
--   slides' CSV / JSON examples).
...
-- 5. Expressions.
--
--    A separate parser per precedence level (top-down, leftmost
--    derivation, à la the slides on LL parsers).  Each level
--    delegates to the next-tighter level for its operands and uses a
--    tail-recursive @rest@ helper to fold left-associative chains.
-- ====================================================================
